...
directory = "/import/c4dm-datasets/nsynth/nsynth-valid/audio/"

##Write directory
write_directory = "/import/c4dm-05/vs/valid-no-reed.tfrecord"
    ...
    with tf.Session(graph=graph) as sess:
        for files in os.listdir(directory):
            if(files.endswith(".wav") and any(word in files for word in word_list)):
                ...
                example = tf.train.Example(features=tf.train.Features(feature={'spec':spec_feature,'shape':shape_feature,'label':label_feature}))
